...
Do $x^{[1]}$ là vector kích thước $d$ x $1$ tuy nhiên ở $X$ mỗi dữ liệu được viết theo hàng nên cần transpose $x^{[1]}$ thành kích thước $1$ x $d$, kí hiệu: $(x^{[1]})^T$
...
\begin{itemize}
\item[$\square$] Ở đây, ta có nhận xét:
...
\begin{figure}[htp]
\begin{center}
\includegraphics[scale=0.15]{chap2/c2_figs/softmax.png}
\end{center}
\caption{Một số ví dụ về đầu vào và đầu ra của hàm softmax}
\label{fig:softmax}
\end{figure}
\FloatBarrier
\centerline{(Nguồn: \url:{https://machinelearningcoban.com})}
