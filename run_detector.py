...
class SkeletonDetector(object):
    ...
    def __init__(self, model="mobilenet_v2_thin"):
...
class ActionClassifier(object):
    ...
    def __init__(self, model_path):
        ...
        self.action_dict = ['xin chao', 'toi', 'thanh pho', 'vui ve', 'am em', 'Sai gon', 'di bo', 'mua mang', 'doi bung', 'yeu', 'an', 'bieu quyet']
    ...
    def predict(self, skeleton):
        ...
        predicted_idx = np.argmax(self.dnn_model.predict(skeleton_input)) #skeleton_input
...
if __name__ == "__main__":
    ...
    if DO_INFERENCE:
        classifier = ActionClassifier(
            CURR_PATH + "../model/sign_language_12_weak.h5" #sign_language.h5   action_recognition_21-10.h5
        )
    ...
    while ith_img <= images_loader.num_images:
        ...
        for ith_skel in range(0, len(skelsInfo)):        #moi nguoi #len(skelsInfo)
            print('Day neeeeeeeeeeee',len(skelsInfo))  #1
            ...
            if DO_INFERENCE and len(skeleton_choose) == 20: #and len(skeleton_choose) == 20
                prediced_label = classifier.predict(skeleton_choose)
